...
if __name__ == '__main__':
    ...
    k_to_save_dir = r'k_to_save//真的梯度下降法分量各自迭代保存的系数结果//2020-8-21//'
    update_time_tosave = 500
    # -------------------------------------迭代后的系数保存位置，多少次迭代保存一次更新的系数矩阵 ----------每次要修改---------
    makedir(k_to_save_dir)
    # 初始化变量f ，全局变量
    eplison = 1e-4# 小于该误差停止迭代
    max_iter = 1e10  # 大于该循环次数停止迭代
    learning_rate = 0.0001  # 每次某一个系数改变的大小，方向正或者负 , 学习率
    ...
    # 设置的点的个数，一个中心点， 26*2个零点, 共26*2+1个预设点
    ...
    while dist_min > eplison:  # 误差小于预设误差，退出迭代
        ...
        # sign_to_change = np.ones(shape=(3,40,1))   # 1代表正向改变 ， -1代表负向改变
        sign_to_change = np.ones(shape=(mode_fenliang ,mode_number ,2,1))
        # 2代表  实数部分 或虚数部分
        # sign_to_change=1
                    ...
                    if(realorimg == 0): #实数部分改变
                        ...
                        if (dist_new_postive < dist_min or dist_new_negative < dist_min):
                            ...
                            if (dist_new_postive < dist_new_negative):
                                real_or_img =  0
                                sign_to_change[exeyez,mode ,realorimg,0] = 1
                                k_to_change = mode
        ...
        if (dist_min < dis_old):
            print('dist new is ',dist_min,
                  '次数是 ', count,)
            # ---------------------------你是猪吗？？？？？？ ？？？？？？？ ？？？？？？？？？？？？？？？？？？？？下面是错的
            for num in range(mode_number):
                for exeyez in range(mode_fenliang):
                    k_ex_ey_ez[exeyez , num , 0] = k_ex_ey_ez[exeyez , num , 0] + learning_rate*sign_to_change[exeyez, num, 0, 0]*1.0 +    learning_rate*sign_to_change[exeyez, num, 1, 0]*1.0 *(1j)
            ...
            # 每500次保存一次数据
            if (count % update_time_tosave == 0):
                print('第',count,'次循环',k_ex_ey_ez)
    ...
    np.save('k_to_save//真的梯度下降法dist_record.npy',dist_record)
