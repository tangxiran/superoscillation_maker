...
if __name__ == '__main__':
    # 貌似做小dist 为0.28？？
    # 迭代后的系数保存位置， ----------每次要修改---------
    k_to_save_dir = 'k_to_save//总场强esum非分量计算长的强度设置为10000//时间2020-08-16//'
    ...
    learning_rate_dict ={ 'time1' : 10 , 'time2' : 0.001,'time3':0.0001,'time4':0.0001*0.1}
    learning_rate_dict_yuzhi = {'yuzhi1' : 100 , 'yuzhi2' : 10,'yuzhi3':1,'yuhzi4':0.1}

    # 中心点强度
    zhongxingdianqiangdu = 1.0 * 10000
    ...
    while dist>eplison: # 误差小于预设误差，退出迭代
        ...
        # 每1000次保存一次数据
        if(count % count_to_save ==0):
            print(k)
            saveplace = k_to_save_dir+'No'+ str(count)+'.npy'
            np.save(saveplace , k)
        if dist_old==dist_min : print('迭代求解结束，最终迭代次数为',count,'最终距离为',dist_min,'初始距离为：,',dist_origin  ); break ;
