...
if __name__ == '__main__':
    # 貌似做小dist 为0.28？？
    # 迭代后的系数保存位置， ----------每次要修改---------
    k_to_save_dir = 'k_to_save//增加点的个数总场强esum非分量计算长的强度设置为100//时间2020-08-16//'
    ...
    # learning_rate_dict ={ 'time1' : 0.1 , 'time2' : 0.001,'time3':0.0001,'time4':0.0001*0.1}
    # learning_rate_dict_yuzhi = {'yuzhi1' : 100 , 'yuzhi2' : 10,'yuzhi3':1,'yuhzi4':0.1}
    ...
    for i in range(1):
        ...
        # 设置的点的个数，一个中心点， 26*2个零点, 共26*2+1个点
        numberofPreset = 3  *banfenkuang_int - banfenkuang_int +1
    ...
    while dist>eplison: # 误差小于预设误差，退出迭代
        # if dist>= learning_rate_dict_yuzhi['yuzhi1']:learning_rate=learning_rate_dict['time1']
        # if dist < learning_rate_dict_yuzhi['yuzhi1'] and  dist>= learning_rate_dict_yuzhi['yuzhi2']: learning_rate = learning_rate_dict['time2']
        # if dist < learning_rate_dict_yuzhi['yuzhi2'] and dist >= learning_rate_dict_yuzhi['yuzhi3']: learning_rate = learning_rate_dict['time3']
        # if dist < learning_rate_dict_yuzhi['yuzhi3'] : learning_rate = learning_rate_dict['time4']
        ...
        k_to_change = 0
        # sign_to_change = 1
        sign_to_change = np.ones(shape=(40,1))
        ...
        for temp in range(mode_number):
            ...
            if (dist_new_postive < dist_min or dist_new_negative< dist_min):
                ...
                if (dist_new_postive  < dist_new_negative):
                    sign_to_change[temp,0] = 1

                else:
                    sign_to_change[temp,0] = -1
        for i in range(mode_number):
            k[i,0] = k[i,0] + learning_rate*sign_to_change[i,0]
        dist_min =  distanceFrom_A_To_B(k_X_dots(k , all_point) ,wanted_point)
        print('迭代次数为',count,'distance here new is ', dist_min,)

        # # 每1000次保存一次数据
        if(count % count_of_save ==0):
            print(k)
