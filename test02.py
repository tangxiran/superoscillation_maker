...
if __name__ == '__main__':
    ...
    k_to_save_dir = 'k_to_save//总场强esum非分量计算长的强度设置为100//时间2020-08-16ooo8989oooo//'

    makedir(k_to_save_dir)

    # 初始化变量f ，全局变量
    eplison = 1e-4  # 小于该误差停止迭代
    max_iter = 2000000  # 大于该循环次数停止迭代
    ...
    while dist > eplison:  # 误差小于预设误差，退出迭代
        ...
        if (sign_to_change == 1):
            print('迭代次数是', count, ',要改变的模式系数是第', k_to_change + 1, '个', '当前的dist是', dist_min, '要改变的符号方向是 + ',
                  learning_rate)
        if (sign_to_change == -1):
            print('迭代次数是', count, ',要改变的模式系数是第', k_to_change + 1, '个', '当前的dist是', dist_min, '要改变的符号方向是 - ',
                  learning_rate)
    ...
    dist_record_np = np.array(dist_record)
    loss_saveplace = 'k_to_save//2wloss_record-02.npy'
    np.save(loss_saveplace, dist_record_np)
